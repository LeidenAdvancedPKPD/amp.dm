...
This is an adaptation of the original Theophylline dataset where additional subjects, dose arms and covariates are added. With this data, source data is created that can be used to demonstrate how the pacakge works.
...
# Save all data frames in environment as Rdata object
# savelist  <- unlist(lapply(ls(), function(x) if(is.data.frame(get(x))) return(x)))
# save(list=savelist,file=paste0(get_script(),".RData"))
```
...
The tables in this section are useful for revieing and documenting the data management process. The following functions are used:

- `define_tbl`; Uses the meta data or attributes to create a define table that can also be used for eSubmission
- `stats_df`; Provide simple statistics of available data to spot for instnace outliers and missing data 
...
- `check_nmdata`; Provide checks for NONMEM data either essential or triggering futher investigation
...
```{r data_stats, results="asis", warning=FALSE, message=FALSE}
...
```{r  log_overview, results="asis", warning=FALSE, message=FALSE}
all_log   <- get_log()
...
```{r check_nmdata, results="asis", warning=FALSE, message=FALSE}
...
```{r session_table, results="asis", warning=FALSE, message=FALSE}
session_tbl()
```